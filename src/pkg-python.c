...

{
    ...
    python_type_entry->efun[efun_idx].exists = true;
            ...
            switch (type->t_object.type)
            {
                case OBJECT_REGULAR:
                    if (type->t_object.program_name == NULL)
                    {
                        ...
                        return ldmud_concrete_object_type_create(type);
                ...
                case OBJECT_LIGHTWEIGHT:
                    if (type->t_object.program_name == NULL)
                    {
                        ...
                        return ldmud_concrete_lwobject_type_create(type);
            }
            break;
    ...
    for (int idx = 0; idx < PYTHON_TYPE_TABLE_SIZE; idx++)
    {
        if (python_type_table[idx] != NULL)
        {
            ...
            for (int efun = 0; efun < REAL_EFUN_COUNT; efun++)
                if (python_type_table[idx]->efun[efun].types)
